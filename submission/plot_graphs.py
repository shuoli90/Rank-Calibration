...
if __name__ == '__main__':
    ...
    ax.grid()
    # put the legend at the bottom of the plot
    # ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.35), ncol=3, fontsize=20, handletextpad=0.1)
    # ax.legend(loc='center right', bbox_to_anchor=(1.2, 0.7), ncol=1, fontsize=30, handletextpad=0.1)
    # ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3, fontsize=25, handletextpad=0.1)
    plt.legend(fontsize=30, loc='center right', bbox_to_anchor=(1.35, 0.5), ncol=1, handletextpad=0.1)
    plt.tight_layout()
    ax.figure.savefig(f'{path}/auroc.pdf', bbox_inches='tight')
    # breakpoint()
    ...
    #               borderaxespad=0.)
    plt.legend()
    plt.xticks(np.arange(len(uncertainty_indicators)), uncertainty_indicators_print, rotation=45)
    plt.xlabel('Uncertainty/Confidence Measure')
